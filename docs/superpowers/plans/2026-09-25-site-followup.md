...
2. Resolve each original `.Params.tags` entry through `.Site.Taxonomies.tags.Get` and use its `.Page.RelPermalink` while rendering the original tag label. This retains duplicate entries and frontmatter order. A focused fixture showed that `.GetTerms "tags"` removes duplicate entries, which can misalign labels by index; `.LinkTitle` also capitalized a lowercase label. The taxonomy method is documented at https://gohugo.io/methods/taxonomy/get/ . Preserve HTML whitespace and every other rendered byte.
